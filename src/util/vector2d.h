...
public:
    ...
    Vector2D(double e0, double e1) {e[0]=e0; e[1]=e1; }
    ...
    double& operator[](int i) { assert ( i >= 0 && i < 2 ); return e[i]; }
    double operator[](int i) const { assert ( i >= 0 && i < 2 ); return e[i];}
    ...
    double length() const { return sqrt(e[0]*e[0] + e[1]*e[1] ); }
    double squaredLength() const { return e[0]*e[0] + e[1]*e[1] ; }
...
inline Vector2D unitVector(const Vector2D& v) {
    double k = 1.0f / sqrt(v.e[0]*v.e[0] + v.e[1]*v.e[1]);
